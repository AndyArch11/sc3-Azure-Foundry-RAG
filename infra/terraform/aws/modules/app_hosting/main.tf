  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        { name = "SEARCH_INDEX_NAME", value = var.search_index_name },
        { name = "CONTROLS_INDEX_NAME", value = var.controls_index_name },
        { name = "OPENSEARCH_GROUNDING_INDEX_KNN_ENABLED", value = tostring(var.opensearch_grounding_index_knn_enabled) },
        { name = "OPENSEARCH_GROUNDING_EMBEDDING_DIMENSIONS", value = tostring(var.opensearch_grounding_embedding_dimensions) },
        { name = "GROUNDING_EMBED_ON_INGEST", value = tostring(var.grounding_embed_on_ingest) },
        { name = "DYNAMODB_TABLE", value = var.dynamodb_table_name },
        { name = "BEDROCK_MODEL_ID", value = var.bedrock_model_id },
        { name = "BEDROCK_EMBEDDING_MODEL_ID", value = var.bedrock_embedding_model_id },
