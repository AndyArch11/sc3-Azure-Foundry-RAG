...
- If Foundry project capability host creation intermittently fails after role assignment changes, consider adding an explicit short `time_sleep` dependency between role assignments and capability host resources to absorb Entra/RBAC propagation delay.
...
- API Management v2 is deployed as Standard v2 with outbound virtual network integration and a separate inbound private endpoint on the gateway subresource.
- APIM API and MCP endpoint definitions are managed separately from the APIM service through `mcp_endpoints`.
...
### 2) Hardened storage account requires Entra auth in Terraform provider
...
- Ensure provider uses Entra auth by setting `storage_use_azuread = true` in the `azurerm` provider.
