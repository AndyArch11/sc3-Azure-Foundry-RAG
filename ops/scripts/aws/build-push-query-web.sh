...
if [[ -z "${ECR_REPO_URL}" ]] && command -v terraform &>/dev/null; then
  ...
  ECR_REPO_URL="$(terraform output -raw query_web_ecr_repository_url 2>/dev/null || true)"
  popd >/dev/null
fi

# Fall back to naming convention: <account>.dkr.ecr.<region>.amazonaws.com/<project>-query-web-<env>-<region_short>
if [[ -z "${ECR_REPO_URL}" ]]; then
  ACCOUNT_ID="$(aws sts get-caller-identity --query Account --output text)"
  ECR_REPO_URL="${ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/rag-query-web-${ENV}"
