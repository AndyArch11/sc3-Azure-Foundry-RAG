...
# Run Azure graph snapshot smoke checks for query web from a private-network-connected location.
# Supports optional build+publish verification followed by Azure-backed export/node/related reads.
...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<'EOF'
Usage:
  ./ops/scripts/azure/run-query-web-graph-smoke.sh <base_url> [auth_token]

Examples:
  QUERY_GRAPH_NODE_ID="a:nist-csf-gv-gv-oc-01" \
    ./ops/scripts/azure/run-query-web-graph-smoke.sh "https://<fqdn>" "<token>"
